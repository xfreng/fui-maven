	...
	$(function() {
		$('#calendar').fullCalendar({
			...
			select: function( startDate, endDate, allDay, jsEvent, view ){
				...
				$.fancybox({
					'type':'ajax',
					'href':'<%=request.getContextPath() %>/supervisor/eventopt?action=add&date='+start+'&end='+end
				});
			},
			editable: true,
			dragOpacity: {
			...
			eventDrop: function(event,dayDelta,minuteDelta,allDay,revertFunc) {
				$.post("<%=request.getContextPath() %>/supervisor/event?action=drag",{id:event.id,daydiff:dayDelta,minudiff:minuteDelta,allday:allDay},function(msg){
					if(msg!=1){
						alert(msg);
						revertFunc(); //恢复原状
					}
			...
			eventResize: function(event,dayDelta,minuteDelta,revertFunc) {
				$.post("<%=request.getContextPath() %>/supervisor/event?action=resize",{id:event.id,daydiff:dayDelta,minudiff:minuteDelta},function(msg){
					if(msg!=1){
						alert(msg);
						revertFunc();
					}
			...
			events: '<%=request.getContextPath() %>/supervisor/getCalendarJsonData',
			dayClick: function(date, allDay, jsEvent, view) {
				var selDate =$.fullCalendar.formatDate(date,'yyyy-MM-dd');
				$.fancybox({
					'type':'ajax',
					'href':'<%=request.getContextPath() %>/supervisor/eventopt?action=add&date='+selDate
				});
			},
			eventClick: function(calEvent, jsEvent, view) {
				$.fancybox({
					'type':'ajax',
					'href':'<%=request.getContextPath() %>/supervisor/eventopt?action=edit&id='+calEvent.id
				});
			}
		});
	});
