...
var wendangController = [ '$scope', '$modal','$rootScope', function($scope, $modal,$rootScope) {
	var opts = {
			template:  'editor-app/configuration/properties/wendang-text.jsp?version=' + Date.now(),
			scope: $scope
	};
	// Open the dialog
	$modal(opts);
...
var wendangControllerCtrl = ['$scope', function($scope) {
	...
	$scope.iframesrc = $scope.action+"/DispatchAction.do?efFormEname="+formEname;
    $scope.save = function() {
...
var callelementController = [ '$scope', '$modal','$rootScope', function($scope, $modal,$rootScope) {
	var opts = {
			template:  'editor-app/configuration/properties/callelement-text.jsp?version=' + Date.now(),
			scope: $scope
	};
	// Open the dialog
	$modal(opts);
...
var callelementControllerCtrl = ['$scope', function($scope) {
	...
	$scope.iframesrc = $scope.action+"/DispatchAction.do?efFormEname=RWPE102&inqu_status-0-category="+category;
    $scope.save = function() {
    	var iframeObject = document.getElementById("iframepage").contentWindow;
    	var jsonStrValue = iframeObject.getJsonStr();
    	$scope.property.value = jsonStrValue;
