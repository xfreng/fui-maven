...
app.controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    	...
    	$scope.$on('ngRepeatFinished', function(ngRepeatFinishedEvent){
    		...
    		if(tree){
		        /*var selectTreeNode = tree.tree("getSelected");
		...
		function syncTree(id,title,editor){
            ...
            if(bool){
        		...
        		if (shapes && shapes.length == 1){
	        		...
	        		var selectTreeNode = tree.tree("getSelected");
	        		var isLeaf = tree.tree("isLeaf",selectTreeNode.target);
	        		if(isLeaf === true){
	        			selectTreeNode = tree.tree("getParent",selectTreeNode.target);
	        		}
					tree.tree("append", {
					...
					tree.tree("select",tree.tree("find",newid).target);
        		}
            }
		}
        ...
        $scope.updatePropertyInModel = function (property, shapeId) {
            ...
            if (newValue != oldValue) {
                ...
                if(key == "oryx-name"){
                	...
                	var node = tree.tree("find",resourceId);
                	if(node != null){
                		tree.tree("update", {
							target: node.target,
							text: newValue
