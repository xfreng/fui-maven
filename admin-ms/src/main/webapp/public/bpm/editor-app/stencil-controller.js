...
app.controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {

    // 加载完后处理事件 -- edit by sf.xiong
    $scope.$on('ngRepeatFinished', function (ngRepeatFinishedEvent) {
        var tree = parent.tree;
        if (tree) {
            var selectTreeNode = tree.getSelectedNode();
            if (selectTreeNode != null) {
                parent.changeHighlightNode(selectTreeNode.id);
                tree.select(selectTreeNode);
            }
        }
    });

    // 初始化参数列表   -- edit by sf.xiong
    $scope.gridData = {'rows': []};
    $scope.itemGridData = {'rows': []};
    // Split div toggle state -- edit by sf.xiong
    $scope.splitIconState = {'collapsed': false};
    ...
    // 点击改变左右宽度    -- edit by sf.xiong
    $scope.splitIconState.toggle = function () {
        $scope.splitIconState.collapsed = !$scope.splitIconState.collapsed;
        $timeout(function () {
            if ($scope.splitIconState.collapsed) {
                jQuery("#paletteHelpWrapper").css("display", "none");
                jQuery("#canvasHelpWrapper").css("width", "100%");
                jQuery("#propertiesHelpWrapper").css("width", "100%");
            } else {
                jQuery("#paletteHelpWrapper").css("display", "block");
                jQuery("#canvasHelpWrapper").css("width", "80%");
                jQuery("#propertiesHelpWrapper").css("width", "80%");
            }
        });
    };
    ...
    // 判断节点类型是否展示到树节点上    -- edit by sf.xiong
    function checkTypeShow(typeId) {
        var bool = false;
        var types = [];
        types.push("UserTask");
        types.push("ServiceTask");
        types.push("ScriptTask");
        types.push("BusinessRule");
        types.push("ReceiveTask");
        types.push("ManualTask");
        types.push("MailTask");
        types.push("CamelTask");
        types.push("MuleTask");
        types.push("SubProcess");
        types.push("EventSubProcess");
        types.push("CallActivity");
        for (var i = 0; i < types.length; i++) {
            var type = types[i];
            if (typeId.indexOf(type) != -1) {
                bool = true;
                break;
            }
        }
    ...
    // 同步树      -- edit by sf.xiong
    function syncTree(id, title, editor) {
        var tree = parent.tree;
        if (id == "UserTask") {
            title = "人工活动";
        }
        var bool = checkTypeShow(id);
        if (bool) {
            var shapes = editor.getSelection();
            if (shapes && shapes.length == 1) {
                var currentSelectedShape = shapes.first();
                var newid = currentSelectedShape.resourceId;
                var selectTreeNode = tree.getSelectedNode();
                var isLeaf = tree.isLeaf(selectTreeNode);
                if (isLeaf === true) {
                    selectTreeNode = tree.getParentNode(selectTreeNode);
                }
                var newNode = {id: newid, text: title};
                tree.addNode(newNode, "add", selectTreeNode);
    ...
    $scope.editorFactory.promise.then(function () {
        ...
        $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
            ...
            for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
                ...
                var stencilItem = {
                    ...
                    'params': data.stencils[stencilIndex].params,// -- edit by sf.xiong
        ...
        $scope.deleteShape = function () {
            ...
            $scope.gridData.rows = [];// 清空原有赋值   -- edit by sf.xiong
        ...
        $scope.quickAddItem = function (newItemId) {
            $scope.safeApply(function () {
                ...
                if (shapes && shapes.length == 1) {
                    ...
                    // 有设置params参数的节点进行赋值      -- edit by sf.xiong
                    var item = $scope.getStencilItemById(containedStencil.title());
                    if (item.params != null && typeof(item.params) != "undefined") {
                        command.shape.properties["oryx-documentation"] = item.params;
                    }
                    //用于刷新一次属性编辑页面 -- edit by sf.xiong
                    $scope.editor.setSelection([null]);
                    $scope.editor.setSelection([command.shape]);
                    // 同步树      -- edit by sf.xiong
                    var typeTitle = containedStencil.title();
                    syncTree(newItemId, typeTitle, $scope.editor);
                }
            });
        };
    ...
    $scope.updatePropertyInModel = function (property, shapeId) {
        ...
        if (newValue != oldValue) {
            ...
            if (key == "oryx-name") {
                ...
                if (node != null) {
                    tree.updateNode(node, {text: newValue});
                }
            }
        } else {
    ...
    $scope.dropCallback = function (event, ui) {
        ...
        if ($scope.dragCanContain) {
            // 根据类型对应的中文名称查询(避免类型id重复) -- edit by sf.xiong
            // var item = $scope.getStencilItemById(ui.draggable[0].id);
            var item = $scope.getStencilItemById(ui.draggable[0].textContent.trim());
            ...
            if ($scope.quickMenu) {
                ...
                var typeTitle = ui.draggable[0].textContent.trim();
                syncTree(typeId, typeTitle, $scope.editor);
