function graphTrace(options) {
	 ...
	 */
	$('body').on('click', '#diagram-viewer', function() {
		$('#workflowTraceDialog').dialog('close');
		...
		var url = ctx + '/bpm/diagram-viewer/index.html?processDefinitionId=' + opts.pdid + '&processInstanceId=' + opts.pid;
    ...
    $.getJSON(ctx + '/workflow/process/trace?pid=' + opts.pid, function(infos) {
        ...
        if ($('#workflowTraceDialog').length == 0) {
            $('<div/>', {
                ...
                title: '查看流程&nbsp;<button id="changeImg">如果坐标错乱请点击这里</button><button id="diagram-viewer">Diagram-Viewer</button>',
                html: "<div><img src='" + imageUrl + "' style='position:absolute; left:0px; top:"+ _top +"px;' />" +
