 ...
 * @param {EFTree} tree 树对象
 * @return {Object} 返回节点的父节点数据(JSON)或者null
 */
eiTreeModel.prototype.getParent = function(p,tree){
    var ret = null;
    var host = this;
    var queryCallBack = { 
		 onSuccess: function(ei){			
...
/**
 * 后端相应接口，从后台查找某个节点
 * @param field {String}  在哪个字段查找
 * @param key   {String} 查找的关键字
 * @return {EFTTNode} 查找的节点
 */
eiTreeModel.prototype.getNodes = function(field,key){
	var ret = null;
	//先查看是否有缓存
    var host = this;
    var queryCallBack = { 
		 onSuccess: function(ei){		
    	     host.status(0);
			 var blocks = ei.getBlocks();
			 var _block = blocks[host.resultBlockName];
    ...
    var columnField = new EiColumn(EFTree.config.TREE_SEARCH_FIELD);
    columnField.pos = 0;
    queryMeta.addMeta(columnField);
    
	var columnKey = new EiColumn(EFTree.config.TREE_SEARCH_KEY);   
	columnKey.pos = 1;
    queryMeta.addMeta(columnKey);
	
	var queryBlock = new EiBlock(queryMeta);
	queryBlock.addRow([field+"",key+""]);
//	queryBlock.addRow([key+""]);
    ...
    //同步调用  methodQuery默认调用后台Service的query
    EiCommunicator.send(this.service, "getNodes", eiinfo, queryCallBack, false, null, true);	
	return ret;
};

/**
*指定数据模型中树图标所在的字段
*@ return {String} 数据模型中树图标所在字段名
*/
eiTreeModel.prototype.eiIcon = function(v){ 
	if (undefined == v) {return this._eiIconSrc;};
	this._eiIconSrc = v;
};
...
*@ return {String} 数据模型中树图标所在字段名
*/
tagTreeModel.prototype.eiIcon = function(v){ 
	if (undefined == v) {return this._eiIconSrc;};
	this._eiIconSrc = v;
};
...
*@ return {String} 数据模型中树图标所在字段名
*/
xmlTreeModel.prototype.eiIcon = function(v){ 
	if (undefined == v) {return this._eiIconSrc;};
	this._eiIconSrc = v;
};
...
*@ return {String} 数据模型中树图标所在字段名
*/
absTreeModel.prototype.eiIcon = function(v){ 
	if (undefined == v) {return this._eiIconSrc;};
	this._eiIconSrc = v;
};
 ...
 * @return {Number} 整数。0:成功 -1:失败
 */
absTreeModel.prototype.status = function (v) {
    if (v === undefined) { return this._status; }
    this._status = v;
}
