...
var EiCommunicator = {
   send   :  
     function(sService, sMethod, sEiInfo, sCallback, hasForm, ajaxMode, noloading) {
		var modalWin = null;
		var resizeFunc = null;
		if(document.forms.length>0 && !noloading){
          ...
          $.ajax(
          {
             ...
             // data: "service=" + sService + "&method=" + sMethod +
				// "&eiinfo=" + jsonEi,
    ...
    sendRemote   :
            function(sRemoteUrl,sRemoteKey,sService, sMethod, sEiInfo, sCallback, hasForm, ajaxMode, noloading) {
                var modalWin = null;
                var resizeFunc = null;
                if(document.forms.length>0 && !noloading){
                    resizeFunc = window.onresize;
   ...
   // func call
   sendFuncCall	:
     function(funcId, inInfo, callback) {
   ...
   $send   :  
     function(sUrl, sContent, sCallback) {    
	  // 判断是否超时
